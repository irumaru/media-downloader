...
mise run gen:go:ogen    # OpenAPI → Go
mise run gen:go:sqlc    # SQL → Go
```
...
│   │   ├── models.go      # 生成: 構造体
│   │   └── queries.sql.go # 生成: クエリ関数
│   ├── config/            # 設定ファイル読み込み
│   ├── handler/           # ogen Handler interface の実装
│   ├── service/           # ビジネスロジック (ダウンロード管理)
│   └── worker/            # yt-dlp実行・進捗管理
├── e2e/                   # API E2Eテスト (runn)
│   ├── downloads.yaml
│   └── secret_link.yaml
├── frontend/
